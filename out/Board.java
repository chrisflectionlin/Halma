...
public class Board {
    ...
    /*
    public ArrayList<Board> newJump(Piece piece){
        ArrayList<Board> ans = new ArrayList<>();
        while(!jumpExist(piece.getY(),piece.getX(),piece)){
    ...
    public ArrayList<Board> jump(Piece piece, ArrayList<Board> boards,Board current,Piece original){
        ...
        if(!jumpExist(piece.getY(),piece.getX(),original,boardarr)){
            return boards;
        }
        int piece_y = piece.getY();
        int piece_x = piece.getX();
        ...
        //right
        if(piece_x+2<this.width&&!boardarr[piece_y][piece_x+1].equals(".")
                && boardarr[piece_y][piece_x+2].equals(".")
                && original.getX()!=piece_x+2){
            Board newmove = current.newBoard(piece,piece_y,piece_x+2);
            newmove.setJumped(true);
    ...
    public boolean jumpExist(int piece_y, int piece_x,Piece original, String[][] boardarr){
        //jump up left
